...
if __name__ == '__main__':
    ...
    parser.add_argument('--g_batch', type=int, default=10000, help="batch size of GNN method")
...
if args.norelation:
    ...
    test_g.edata['feats'] = torch.randn(edata.size()).to(args.gpu)
    edata = dev_g.edata['feats']
    dev_g.edata['feats'] = torch.randn(edata.size()).to(args.gpu)

if args.nonode:
    ndata = train_g.ndata['feats']
    train_g.ndata['feats'] = torch.randn(ndata.size()).to(args.gpu)
    ndata = test_g.ndata['feats']
    test_g.ndata['feats'] = torch.randn(ndata.size()).to(args.gpu)
    ndata = dev_g.ndata['feats']
    dev_g.ndata['feats'] = torch.randn(ndata.size()).to(args.gpu)
